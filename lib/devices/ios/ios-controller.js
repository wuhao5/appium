...
iOSController.backgroundV9 = function (appName, secs, cb) {
  this.proxy(["try{au.background("
         , secs
         , ");}catch(e){"
         , "target.tap(target.frontMostApp().mainWindow().scrollViews()[0].elements()['"
         , appName
         , "'].hitpoint());}"
         ].join(''),cb);
};
  ...
  if (v >= 9) {
    if (!this.args.appName) {
      this.getAppNameFromApp(function (err, appName) {
        if (err) return cb(err);
        this.backgroundV9(appName, secs, cb);
      });
    } else {
      this.backgroundV9(this.args.appName, secs, cb);
    }
  } else {
