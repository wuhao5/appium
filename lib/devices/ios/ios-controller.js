...
iOSController.backgroundV9 = function (secs, cb) {
  var thiz = this;
  thiz.proxy(["$.background(", secs, ")"].join(''), function (err, ret) {
    if (err) return cb(err);
    var noThrows = ret && ret.status == 0;
    setTimeout(function () {
      var cmd = "";
      // throws an exception if the app is not launched
      cmd += "if ($.bundleId() == 'com.apple.springboard') {\n";
      // background doesn't throw an exception but it could also fail to relaunch
      // so try to launch safari to help bring up the backgrounded apps
      if (noThrows) {
        cmd += "  $.target().tap($.mainWindow().buttons()['Safari'].hitpoint());\n";
      }
      cmd += "  throw new Error('fail to relaunch');\n";
      cmd += "}\n";
      thiz.proxy(cmd, function (err, ret) {
        if (err) return cb(err);
        if (ret.status != 0) {
          setTimeout(function() {
            var cmd = "";
            cmd += "try {$.target().deactivateAppForDuration(0.1);} catch (e) {};\n";
            cmd += "var apps = $.mainWindow().scrollViews()[0].elements().toArray();\n";
            cmd += "$.target().tap(apps[apps.length - ";
            cmd += (noThrows ? 3 : 2); // safari would be the first item if noThrows
            cmd += "].hitpoint());\n";
            thiz.proxy(cmd, cb);
          }, noThrows ? 1000 : 1); // if throws, no need to wait for safari to launch
        } else {
          cb (null, ret);
        }
      });
